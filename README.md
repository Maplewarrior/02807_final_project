...
> [!WARNING]
> Be aware that the models are large which means that running the experiments on the CPU will still require some memory. We are **not** able to load all the models on a Surface Laptop 3 with 8 GB RAM and an `Intel(R) Core(TM) i5-1035G7 CPU @ 1.20GHz`. Even with prebuilt data.
> **A more powerful setup is required.**
